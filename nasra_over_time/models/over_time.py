...
class OverTime(models.Model):
    ...
    def constraint_limit_time(self):
        limit_over = self.env['over.time.configuration'].search([],order='create_date desc',limit=1).over_time_limit
        date_from_datetime = datetime.strptime(str(self.date_from), '%Y-%m-%d %H:%M:%S') + timedelta(hours=2)
        date_out_datetime = datetime.strptime(str(self.date_to), '%Y-%m-%d %H:%M:%S') + timedelta(hours=2)
        if self.date_from and self.date_to:
            diff = abs(self.time_to_float(date_from_datetime.time()) - self.time_to_float(date_out_datetime.time()))
            if diff < limit_over:
                raise ValidationError(_('Cannot Save over time it doesnt excced limit'))
