...
class HrExcuse(models.Model):
    ...
    approbations = fields.One2many('hr.employee.excuse.approbation', 'excuses', string='Approvals', readonly=True)
    ...
    def approve(self):
        for excuse in self:
            ...
            excuse = excuse.sudo()
            approvers_0day = excuse.employee_id.holidays_approvers.filtered(lambda d: d.max_allow_days == 0).sorted(
                key=lambda x: x.sequence)
            approvers_days = excuse.employee_id.holidays_approvers.filtered(
                lambda d: d.max_allow_days != 0 and d.max_allow_days < excuse.number_of_days_temp).sorted(
                key=lambda x: x.max_allow_days)

            if approvers_0day:
                for index, appr in enumerate(approvers_0day):
                    if excuse.pending_approver.id == appr.approver.id:
                ...
                for indexes, apprs in enumerate(approvers_days):
                    if excuse.pending_approver.id == apprs.approver.id:
                ...
                for indexess, apprss in enumerate(approvers_days):
                    if excuse.pending_approver.id == apprss.approver.id:
            ...
            if is_last_approbation:
                ...
                excuse.write({'state': 'approve', 'pending_approver': next_approver and next_approver.id or False})
                self.env['hr.employee.excuse.approbation'].create(
                    {'excuses': excuse.id, 'approver': self.env.uid, 'sequence': sequence,
    ...
    def validate(self):
        ...
        for excuse in self:
            self.env['hr.employee.excuse.approbation'].create(
                {'excuses': excuse.id, 'approver': self.env.uid, 'date': fields.Datetime.now()})
